...
`etl` works with a local or remote database to store your data. Every `etl` object extends a `dbplyr::src_dbi` object. If, as in the example above, you do not specify a SQL source, a local `RSQLite` database will be created for you. However, you can also specify any source that inherits from `dbplyr::src_dbi`.
